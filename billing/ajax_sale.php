...
if(isset($_POST['bar_code']) AND !empty($_POST['bar_code'])){
	    ...
	    $sqlCheck = $conn->query("SELECT invoice_id FROM invoices WHERE invoice_id = '".$_POST['invoice_id']."'");
    	...
    	if($numCheck <= 0){
        	...
        	$user_sql = $conn->query("SELECT name FROM users WHERE id = '".$_SESSION['user_id']."'");
        	...
        	for($i= 0;$i < sizeof($_POST['bar_code']);$i++){
        		if(!empty($_POST['bar_code'][$i])){
        			...
        			$sqlH = $conn->query("SELECT hsn_code,igst FROM product WHERE bar_code = '".$_POST['bar_code'][$i]."'");
        			$rowH = $sqlH->fetch_array();
        			
        			$conn->query("UPDATE product SET unit = (unit - '".$_POST['quantity'][$i]."') WHERE name = '".$_POST['name'][$i]."'");
        	...
        	$sqlS = $conn->query("SELECT igst,SUM(sale_price) AS sum_sale_price FROM sale WHERE invoice_id = '".$_POST['invoice_id']."' GROUP BY igst");
        	...
        	$conn->query("INSERT INTO invoices VALUES('','".$_POST['invoice_id']."','".$_POST['user_id']."','".round($sum)."','".$_POST['payment_method']."','".date('Y-m-d H:i:s')."')");
        	if($conn->insert_id){
        		echo $printData;
        	}
    	}else{
